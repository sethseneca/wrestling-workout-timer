...
test("uses one boosted whistle with a mixable Web Audio session", async () => {
  ...
  assert.ok(harness.fetchUrls.some((url) => url.includes("assets/audio/ten-second-clapper.m4a?v=20260804-original-clap1")));
...
test("a 10-minute session keeps its audio graph alive for the full running lifecycle", async () => {
  ...
  harness.api.state.targetWallTime = Date.now() + 9000;
  harness.api.tick();
  const fightClapperSequences = audioContext.startedSources.filter(
    (source) => source.buffer === harness.api.state.audioBuffers.tenSecondClapper
  );
